...
qauntidade_casos = int(input("Quantidade de casos de teste: "))

for _ in range(qauntidade_casos):
    populacao_A = int(input("População A: "))
    populacao_B = int(input("População B: "))

    taxa_crescimento_A = float(input("Crescimento A: "))
    taxa_crescimento_B = float(input("Crescimento B: "))
    ...
    while populacao_A <= populacao_B:
        ...
        if anos > 100:
            print("Total de anos: Mais de 1 seculo")
            break
    else:
        print(f"Total de anos: {anos} anos")
